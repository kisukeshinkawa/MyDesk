...
> ⚠️ **先に [VERIFICATION.md](./VERIFICATION.md) を読んでください。**
> 検証の結果、この売買ロジックは**株価指数に勝てませんでした**。
> 以下の設計は実装されていますが、「それで儲かる」部分は支持されていません。
> 銘柄スコア・チャート・ニュース・損切り/利確の目安は、
> 自分で判断するための道具としてはそのまま使えます。

MyDeskから分離した独立アプリ。国内株・米国株の分析(スコアリング/AI判定/学習/ポートフォリオ管理)。
